    ...
    <div class="bg-gray-100">
        ...
        @forelse ($comments as $comment)
            <div class="w-11/12 md:w-4/5 lg:w-3/5 mx-auto mb-3 p-4 md:p-6 bg-gray-50 shadow-md rounded-lg">
                <p class="text-cyan-700 mb-3 text-md"><a href="{{ route('user.show', $comment->user->username) }}" class="hover:underline">{{ optional($comment)->user->username }}</a> <span class="ml-3 text-gray-700 text-sm">Posted {{ Carbon\Carbon::parse(optional($comment)->created_at)->diffForHumans() }}</span></p>
        ...
        @empty
             <div class="w-11/12 md:w-4/5 lg:w-3/5 mx-auto mb-3 p-4 md:p-6 bg-gray-50 shadow-md rounded-lg flex flex-col justify-center items-center">
                <img src="{{ asset('img/svg/feedback.svg') }}" alt="A girl writing something into a laptop" class="block h-1/2 w-1/2 md:h-1/4 md:w-1/4 mx-auto mb-5">
                <h6 class="font-bold">No Comments Yet</h6>
        ...
        @auth
            <div class="w-11/12 md:w-4/5 lg:w-3/5 mt-5 mx-auto p-4 md:p-6 bg-gray-50 shadow-md rounded-lg">
                <form wire:submit.prevent="createComment">
                    ...
                    <textarea name="body" wire:model="body" rows="5" placeholder="Write a comment" class="w-full border-gray-300 focus:border-cyan-500 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 rounded-md shadow-sm"></textarea>
